...
name_of_experiment = 'test_train'
ex = Experiment(name_of_experiment)


@ex.main
def run():
    logger = config.get_logger(name_of_experiment)
    ...
    epoch = 0
    with tqdm(zip(*data_loader), desc=f"Training epoch {epoch}", total=100) as progress:
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                if tokenizer is not None:
                    data['text'] = tokenizer(data['text'], return_tensors='pt', padding=True,
                                                    truncation=True)
                data['text'] = {key: val.to(trainer.device) for key, val in data['text'].items()}
                # data['video'] = data['video'].to(device)
                text_embeds = model.compute_text(data['text'])
